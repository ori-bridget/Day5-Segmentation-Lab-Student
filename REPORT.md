...
Kiểm tra cấu trúc export: **9/9 task OK**, đúng ảnh, class và dạng mask. Self-check với ground truth ba tier: `easy_semantic` **20.0/20**, `medium_instance` **10.5/32**, `hard_panoptic` **16.5/30**, tổng **47.0/82**. Sáu checkpoint chưa có ground truth nên chưa tính điểm.
...
- Lỗi thuộc loại: thiếu vật và một số mask dư/sai đối tượng.
- Bằng chứng tôi nhìn thấy: sau khi sửa, scorer đối chiếu 74 annotation với 71 annotation ground truth; `TP = 49`, `FP = 25`, `FN = 22`, recall `0.69`. Điểm Medium tăng từ **4.4/32** lên **10.5/32**. So với reference, `000000373353.jpg` còn thiếu 3 person và có thêm 2 car cùng 4 truck; `000000458325.jpg` còn thiếu 7 person và có thêm 6 car.
- Quy tắc và hành động sửa: đã mở lại ảnh, quét để bổ sung object và giữ từng vật là một mask riêng. Lượt tiếp theo cần xóa các mask dư, kiểm lại car/person/truck theo đúng class rồi export lại.
- Sau sửa đã Save và export lại chưa? **Đã**; `medium_instance.zip` mới có 74 annotation dạng RLE và đã được chấm lại.
